...
struct MRZCode {
    ...
    var mrzString: String {
        guard documentNumberField.isValid
                && birthdateField.isValid
                && expiryDateField.isValid else {
            return ""
        }
        
        let fieldsValidate: [ValidatedFieldProtocol] = [ documentNumberField, birthdateField, expiryDateField ]
        let compositedValue = fieldsValidate.reduce("", { $0 + $1.rawValue + $1.checkDigit })
        return compositedValue
    }
    
    var mrzWithFinalDigit: String {
        guard !self.finalCheckDigit.isEmpty else { return "" }
